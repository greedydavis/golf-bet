...
const DATA_DIR = 'idb://golf-bet-demo';
const IDB_NAME = '/pglite/golf-bet-demo';
const LOCK_NAME = 'golf-bet-demo-db';
const SESSION_KEY = 'golf-bet-demo-user';
...
type RpcAs = <T>(uid: string | null, fn: string, args?: Record<string, unknown>) => Promise<T>;

/**
 * 開啟示範資料庫。只有在 migrations 與示範資料都建立完成後才寫入 ready 標記；
...
async function openDatabase(seed: (db: PGlite) => Promise<void>): Promise<PGlite> {
  ...
  const dirty = await db.query<{ n: number }>(`select count(*)::int as n from pg_namespace where nspname in ('app', 'auth')`);
  if (dirty.rows[0].n > 0) {
    await db.close();
  ...
  for (const key of Object.keys(migrations).sort()) await db.exec(migrations[key]);
  await seed(db);
...
export async function createDemoBackend(): Promise<Backend> {
  ...
  const rpcAs: RpcAs = async <T,>(uid: string | null, fn: string, args: Record<string, unknown> = {}) => {
  ...
  db = await openDatabase(async (target) => {
    db = target;
    await seedDemoData({
      createUser: async (email, displayName) => {
        const res = await target.query<{ id: string }>(`insert into auth.users (email, raw_user_meta_data) values ($1, $2) returning id`, [
          email,
  ...
  return {
    mode: 'demo',
    // 本機開發伺服器（npm run dev）提供 /api/recognize；打包後的示範版沒有伺服器可用
    recognitionAvailable: import.meta.env.DEV,
    ...
    async signUp(email, _password, displayName) {
      try {
        const res = await db.query<{ id: string }>(`insert into auth.users (email, raw_user_meta_data) values ($1, $2) returning id`, [
          email.trim(),
          JSON.stringify({ display_name: displayName }),
        ]);
        userId = res.rows[0].id;
    ...
    async recognize(input) {
      if (!import.meta.env.DEV) throw new RpcError('示範模式無法辨識成績卡，請手動輸入');
      const res = await fetch('/api/recognize', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(input),
      });
      const data = await res.json().catch(() => ({ error: '辨識失敗' }));
      if (!res.ok) throw new RpcError(data.error ?? '辨識失敗');
      return data;
    },

    async listDemoUsers() {
      const res = await db.query<DemoUser>(
        `select p.id, p.display_name, p.role, u.email from app.profiles p join auth.users u on u.id = p.id
         order by array_position(array['owner','member','pending'], p.role), u.created_at`,
      );
      return res.rows;
    },
