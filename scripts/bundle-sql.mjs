...
begin
  if exists (select 1 from pg_namespace where nspname = 'app') then
    raise exception '已經安裝過（app schema 已存在），不要重複執行這份檔案';
  end if;
end $$;`;
...
select
  ...
  (select count(*) from storage.buckets where id = 'scorecards') as scorecard_bucket,
  '安裝完成' as result;`;

const out = join(root, 'supabase', 'setup-all.sql');
writeFileSync(out, `-- 高爾夫賭球結算：Supabase 一次性安裝（自動產生，請勿手動修改）\n\n${guard}\n\n${parts.join('\n\n')}\n\n${finish}\n`);
console.log(`已產生 ${out}`);
